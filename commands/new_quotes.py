...
async def cmd_add_quote(msg: Message, *args):
    ...
    if add_quote(quote):
        resp = f'successfully added quote #{quote.id}, call with !quote {quote.id} or !quote {quote.alias}'
    else:
        resp = 'failed to add quote, already exist'

    await msg.reply(resp)

@Command('randomquote', aliases=['rquote', 'randquote', 'randyquote'], syntax='<ID or ALIAS>', help='gets a quote by ID or ALIAS')
